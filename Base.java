...
public class Base3 extends Application{
	...
	static Transaction recent = new Transaction("","","","","","","","",0);
...

				{
					...
					{		
...

						{
							...
							customer.customerArray[i] = new Customer(customerIn);	
...

	{
		...
		stageTitle.setTitle("Manager Home Screen - Manager: " + managerName);
...

	{
		...
		stageTitle.setTitle("Manager Customize Menu Screen - Manager: " + managerName );
...
//User	The user must enter a double number without the dollar sign		
...

	{
		...
		stageTitle.setTitle("Manager Customer Search Screen - Manager: " + managerName );
...

	{
		...
		submit.setOnAction(submitSelected -> {
			...
			//		customerIn = new Customer(tfName.getText().trim(), tfPhone.getText().trim(), customerIn.custPass, tfAddr.getText().trim(),

			//						customerIn.custAddDesc, customerIn.custCardName, customerIn.custCardNum, customerIn.custCardExp, customerIn.custSecCod, customerIn.custAccount, customerIn.custRoute);
...

	{
		...
		TextField tfmanagerName = new TextField(managerName);

		TextField tfmanagerPass = new TextField(managerPass);
...

	{
		...
		Label cust = new Label(customerIn.custName);

		Label phne = new Label(customerIn.custPhone);

		

		Button date1 = new Button("Order date:"+orderDateString);

//		Button date2 = new Button("Date 2");

//		Button daten = new Button("Date n");
		...
		gPaneTop.add(cust, 0, 0);

		gPaneTop.add(phne, 0, 1);

		gPaneCenter.add(date1, 0, 3);

//		gPaneCenter.add(date2, 0, 6);



//		gPaneCenter.add(daten, 0, 12);
		...
		bpane.setBottom(space);

		

		// Handle Button Action Event
		
		date1.setOnAction(backPressed ->{
			
			bpane.getChildren().clear();

			bpane.setCenter(transactionDisplay());
		});
	...
	public BorderPane customerHistoryManager()

	{

		stageTitle.setTitle("Customer Search Screen - Manager: " + managerName);

		

		Label cust = new Label(customerIn.custName);

		Label phne = new Label(customerIn.custName);

		

		Button date1 = new Button("Date 1");
...

	{
		...
		// Delivery/Pickup Check Box/Price Display
		...
		btSub.setOnAction(submitPressed -> {
			...
			{
				recent.setData(orderDateString, orderSize, orderCrust, orderCheese1 + " " + orderCheese2, orderMeat1 + " " + orderMeat2 + " " + orderMeat3, orderVeg1 + " " + orderVeg2 + " " + orderVeg3, orderBevSize + " " + orderBev, orderDelivery,total);
	...
	public BorderPane receipt() {
		...
		Label lblTotal = new Label("Total: $"+String.valueOf(total));
		...
		btDone.setOnAction(homeSelected -> {
			...
			Text orderConfirm = new Text("Your Order Was Placed at ");
	...
	public BorderPane transactionDisplay() {

		

		stageTitle.setTitle("Most Recent Transaction");
		...
		Label lblSize = new Label("Pizza Size:\t\t\t" + recent.size);

		Label lblCrust = new Label("Crust:\t\t\t\t" + recent.crust);

		Label lblCheese1 = new Label("Cheese:\t\t\t\t" + recent.cheese);

		Label lblMeatTop1 = new Label("Meat Topping:\t\t" + recent.meatTop);

		Label lblVegTop1 = new Label("Veggetable Topping:\t" + recent.vegTop);

		Label lblBev = new Label("Beverage:\t\t\t" + recent.bevSize);

		Label lblDel = new Label("Delivery or Pickup:\t\t" + recent.delOrPick);
		
		Label lblTotal = new Label("Total: $"+recent.total);
...
class Transaction
{
	...
	public String meatTop;
	public String vegTop;
	public String total;
	...
	protected Transaction(String date, String size, String crust, String cheese, String meatTop, String vegTop, String bevSize, String delOrPick, double total)
	{
		this.date = date;
		this.size = size;
		this.crust = crust;
		...
		this.delOrPick = delOrPick;
		this.total = String.valueOf(total);
	}
	
	protected void setData(String date, String size, String crust, String cheese, String meatTop, String vegTop, String bevSize, String delOrPick, double total)
	{
		this.date = date;
		this.size = size;
		this.crust = crust;
